-- Complete fix for multi-seat booking fare calculation (Method A, Logic 2)
...
BEGIN
  ...
  FOR v_segment_idx IN p_from_index..(p_to_index - 1) LOOP
    ...
    IF v_prices_per_segment IS NOT NULL 
      ...
      -- Use segment-specific price (array is 1-indexed in PostgreSQL)
      v_fare := v_fare + COALESCE(v_prices_per_segment[v_segment_idx + 1], v_base_price);
    ELSE
